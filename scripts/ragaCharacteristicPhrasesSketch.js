...
function preload() {
  recordingsList = loadJSON("../files/ragaCharacteristicPhrases-recordingsList.json");
  recordingsInfo = loadJSON("../files/recordingsInfo.json");
  ragInfo = loadJSON("../files/ragaInfo.json");
  talInfo = loadJSON("../files/talaInfo.json");
}
...
function draw () {
  ...
  stroke(frontColor);
  fill(backColor);
  if (ragaName != undefined) {
    var ragW = textWidth(ragaName);
  }
  text(ragaName, extraSpaceW + title_x, extraSpaceH + title_y + title_size);
...
function start () {
  ...
  var raga = ragInfo[currentRecording.raga.name];
  ...
  //       key = "";
  //     }
  //     var svara = new CreateSvara(svaraName, cents, raga.vadi, raga.samvadi, key);
  ...
  phrases = currentRecording.raga.phrases;
  ...
  var tala = currentRecording.tala;
  for (var i = 0; i < tala.sama.length; i++) {
    samaList.push(float(tala.sama[i].toFixed(2)));
  }
